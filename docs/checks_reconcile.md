...
**The unposted 835s.** eCW's Billing → ERA screen lists the electronic remittances the payers sent. Posting Status *UnPosted* is the ones nobody has applied. An ERA proves the payer sent the money; it does not prove the check reached Helixona, and unposted means nobody entered it either (2026-09-21, the operator: "ECW has the 835 files. But Helixona has not confirmed whether they have the checks or not"). The run reads that screen once (`era: true`, on by default for a full run, off for a targeted one) and pages through it; the filter is left as the team has it — Posting Status UnPosted, nothing else touched (the operator, 2026-09-24). **The check number is the TRACE# column** — in an 835 the trace number is the check or EFT the payer issued; the FILE and CHECK columns are eCW's own sequence for the imported file and are kept only so a person can find the row again. An ERA never settles a check: it adds the flag `835 in eCW, not posted`, and where nothing else knows the number the verdict is `835 only`.
