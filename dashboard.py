    ...
    <script>
        ...
        const ACTIONS = {
            ...
            eob: [
                ...
                {icon: '💸', title: 'Refresh the 835s (ERA) only', task: 'check_reconcile',
                 desc: 'Reads Billing → ERA in eCW again and refreshes every verdict with it. Blue Shield, the scans, the tracker and the eCW lookups are not read again — their last answers stand. Minutes, not hours.',
                 payload: {since: '07/01/2025', blue_shield: false, limit_checks: 0, check_eft: '', copies: false, tracker: false, limit_files: 0, ecw: false, era: true}},
                {icon: '🔎', title: 'Look up the unchecked in eCW', task: 'check_reconcile',
                 desc: 'Looks up in eCW → Payments only the checks eCW has never answered for (the "eCW not checked" tile) and re-reads the tracker. Blue Shield, the scans and the 835s are not read again; every earlier eCW answer stands.',
                 payload: {since: '07/01/2025', blue_shield: false, limit_checks: 0, check_eft: '', copies: false, tracker: true, limit_files: 0, ecw: true, ecw_pending: true, era: false}},
                {icon: '🔁', title: 'Compare again (no new reads)', task: 'check_reconcile',
                 desc: 'Re-reads only the tracker spreadsheet and recomputes every verdict from what is already on file. Use it after a rule change. Nothing in Blue Shield, SharePoint scans or eCW is read.',
                 payload: {since: '07/01/2025', blue_shield: false, limit_checks: 0, check_eft: '', copies: false, tracker: true, limit_files: 0, ecw: false, era: false}},
        ...
        function setActiveBot(bot) {
            ...
            if (heroOf) heroOf.hidden = (bot !== 'eob');
            ...
            if (denom) denom.textContent = bot === 'eob'
                ? 'checks compared: copy · Blue Shield · eCW' : 'claims';
            const pctLabel = document.getElementById('hero-pct-label');
            if (pctLabel) pctLabel.textContent = bot === 'eob' ? 'posted & matching' : 'ready to upload';
            const remLabel = document.getElementById('hero-remaining-label');
            if (remLabel) remLabel.textContent = bot === 'eob' ? 'eCW not checked yet' : 'still missing documents';
        ...
        function renderChecks() {
            ...
            if (window.activeBot === 'eob') {
                ...
                const of = document.getElementById('hero-of');
        ...
        function paintClaimsHero(pending, ready) {
            const nf = n => Number(n || 0).toLocaleString('en-US');
            const pct = pending ? Math.round((ready / pending) * 100) : 0;
            const set = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
            set('hero-submitted', nf(pending));
            const lbl = document.getElementById('hero-num-label');
            if (lbl) { lbl.hidden = false; lbl.textContent = pending === 1 ? ' claim to send' : ' claims to send'; }
            const of = document.getElementById('hero-of');
        ...
        function renderClaims(claims) {
            ...
            if (!claims.length) {
                body.innerHTML = searching
                    ...
                    : submittedN ? `<tr><td colspan="8" class="empty-state">Nothing to send — every claim here (${submittedN}) is already sent to SympliSend. See the <a href="/audit">Audit Log</a>.</td></tr>`
