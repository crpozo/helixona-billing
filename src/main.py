...
def process_message(message: dict, aws_client: AWSClient):
    ...
    if _settings.bot_role != 'iv_corrections' and task_type in IV_CORRECTIONS_TASKS:
        ...
        return
    if _settings.bot_role == 'eob' and task_type not in EOB_TASKS:
        logger.warning(f"BOT_ROLE=eob refused task_type={task_type}")
        return
    if _settings.bot_role != 'eob' and task_type in EOB_TASKS:
        logger.warning(f"BOT_ROLE={_settings.bot_role} refused task_type={task_type}")
        return
    ...
    # ──────────────────────────────────────
        ...
        try:
            page = manager.new_page()
            # One shared login for every bot that needs the portal; see
            # src/blueshield/session.py for why it is no longer inline here.
            from src.blueshield.session import login_to_provider_portal
