...
class EachTabShowsOneMainPanel(unittest.TestCase):
    ...
    def test_the_other_tabs_hide_the_eob_table(self):
        self.assertIn("eobSec.hidden = (bot !== 'eob')", _fn('setActiveBot'))
    ...
    def test_the_panels_are_pinned_to_their_columns(self):
        ...
        src = _src()
        self.assertIn('.main > #eob-section, .main > #claims-section-submissions{grid-column:1}', src)
...
class TheHeadlineSpeaksEachTabsLanguage(unittest.TestCase):
    ...
    def test_remittance_counts_captures_not_completions(self):
        fn = _fn('setActiveBot')
        self.assertIn("'captured' : 'complete'", fn)
        self.assertIn("'without an EOB yet' : 'remaining'", fn)


class ACheckOpensOntoTheClaimsItPaid(unittest.TestCase):
    def test_rows_expand(self):
        fn = _fn('loadEobs')
        self.assertIn('class="eob-row', fn)
        self.assertIn('onclick="toggleEob(this)"', fn)
        self.assertIn('eob-claims-table', fn)

    def test_the_breakdown_shows_the_money_per_claim(self):
        fn = _fn('loadEobs')
        for col in ('Billed', 'Paid', 'Pt resp', 'Matched by', 'BSC claim #'):
            self.assertIn(col, fn)

    def test_an_unmatched_claim_says_so(self):
        self.assertIn('unmatched', _fn('loadEobs'))

    def test_opening_the_pdf_does_not_toggle_the_row(self):
        self.assertIn('onclick="event.stopPropagation()"', _fn('loadEobs'))

    def test_open_rows_survive_a_refresh(self):
        self.assertIn('window._eobOpen', _fn('loadEobs'))
        self.assertIn('open.add(key)', _fn('toggleEob'))

    def test_the_eob_list_stays_live_on_its_tab(self):
        self.assertIn("if (window.activeBot === 'eob') loadEobs();", _src())

    def test_the_empty_state_says_what_to_do(self):
        src = _src()
        self.assertIn('Capture EOBs from Blue Shield</strong> from the task panel', src)
